...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if config.task_name == 'Glas':
            mask = cv2.imread(os.path.join(self.output_path, image_filename[: -4] + ".bmp"), 0)
        elif config.task_name == 'BUSI':
            mask = cv2.imread(os.path.join(self.output_path, image_filename[: -4] + ".png"), 0)
        elif config.task_name == 'CVC-ClinicDB':
            mask = cv2.imread(os.path.join(self.output_path, image_filename[: -4] + ".tif"), 0)
        elif config.task_name == 'Kvasir-SEG':
            mask = cv2.imread(os.path.join(self.output_path, image_filename[: -4] + ".jpg"), 0)
        elif config.task_name == 'ISIC201801':
            mask = cv2.imread(os.path.join(self.output_path, image_filename[: -4] + ".png"), 0)
